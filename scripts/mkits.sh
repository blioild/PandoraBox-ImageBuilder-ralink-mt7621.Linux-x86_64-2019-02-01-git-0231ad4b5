...
Generate_FDT () {
	FDT="$FDT
		fdt@$CONFIG_ID {
			description = \"${ARCH_UPPER} PandoraBox ${DEVICE} device tree blob\";
			data = /incbin/(\"${1}\");
			type = \"flat_dt\";
			arch = \"${ARCH}\";
			compression = \"none\";
...
/ {
	description = \"${ARCH_UPPER} PandoraBox FIT (Flattened Image Tree)\";
	#address-cells = <1>;

	images {
		kernel@1 {
			description = \"${ARCH_UPPER} PandoraBox Linux-${VERSION}\";
			data = /incbin/(\"${KERNEL}\");
			type = \"kernel\";
			arch = \"${ARCH}\";
			os = \"linux\";
